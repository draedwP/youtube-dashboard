...
async function load(){
 ...
 try{
  ...
  for(const handle of handles){
   const c=data.channels.find(x=>x.handle===handle);const card=el('article');const heading=el('h2');const link=el('a',c?.title||'@'+handle);link.href='https://www.youtube.com/@'+handle;heading.append(link);heading.style.marginBottom='6px';const subscriberText=c?c.subscribers!=null?`${fmt(c.subscribers)} subscribers`:'Subscriber count hidden':'Subscriber count will appear after the next collection';const subscribers=el('p',subscriberText,'meta');subscribers.style.margin='0 0 24px';card.append(heading,subscribers,el('p','48h view-count increase','label'));
